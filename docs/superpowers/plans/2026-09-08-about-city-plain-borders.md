...
- [ ] Task 1 — city-plan.ts: `CANAL_END`; the canal street, bridges, quay lamps, trees and posts end at the rim; the
...
- [ ] Task 2 — city3d.ts: the living-windows footprint fade (`skinMaterial`). Pane A/B.
- [ ] Task 3 — city3d.ts: the ground's slot filled past `CANAL_END`, water / sheen / quay walls `2 · CANAL_END`, head
...
- [ ] Task 4 — city3d.ts: the filler in the copies, copied trees / kit inside a filler dropped, `plainSpot` on the
      copied glow (heads, lanterns), the corridor cull on masses, glow and paint copies.
- [ ] Task 5 — city3d.ts: the highway's continuation (deck, parapets, edge lights, tubes, piers, arterial strip, lamps).
- [ ] Task 6 — city3d.ts: the far fleet: bodies + light pairs (ring 1 roads / lanes + the continuation within 1,250),
      single lights in ring 2; phones lights only.
- [ ] Task 7 — gates: `npx tsc --noEmit`, `npx vitest run`, `npx vite build`; the pane (desktop path): shimmer A/B,
